...
import { EQUIPMENT_STATUS, PRIORITIES, REQUEST_STATUS, WORK_ORDER_STATUS } from '../utils/constants';
...
const checklistAnswers = ['Bueno', 'Condicional', 'Malo', 'No aplica'];
...
const entityConfig = {
  ...
  requests: {
    ...
    viewPermissions: ['requests.view', 'requests.manage', 'requests.review', 'requests.approve', 'requests.sendToMaintenance'],
    ...
    reviewPermissions: ['requests.review', 'requests.manage'],
    sendToMaintenancePermissions: ['requests.sendToMaintenance', 'requests.manage'],
    collection: 'requests',
...
function ChecklistForm({ value, onChange, errors, equipment }) {
  const setAnswer = (key, patch) => onChange({ ...value, answers: { ...(value.answers || {}), [key]: { ...(value.answers?.[key] || { value: 'Bueno', observation: '' }), ...patch } } });
  const setEquipment = equipmentId => {
    const nextEquipment = selectEquipment(equipment, equipmentId);
    onChange({ ...value, equipmentId, equipmentCode: nextEquipment?.code || '', terminal: value.terminal || nextEquipment?.terminal || '' });
  };
  return <div className="space-y-5">
    ...
    {CHECKLIST_SECTIONS.map(section => <div key={section.title} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="mt-3 space-y-3">{section.items.map(item => {
        const current = value.answers?.[item.key] || { value: 'Bueno', observation: '' };
        return <div key={item.key} className="rounded-lg border border-slate-200 bg-white p-3">
          ...
          {(['Condicional', 'Malo'].includes(current.value) || current.observation) && <textarea className={`${inputClass} mt-3`} rows="2" placeholder="Observación del ítem" value={current.observation || ''} onChange={event => setAnswer(item.key, { observation: event.target.value })}/>}
        </div>;
      })}</div>
    </div>)}
...
function buildPayload({ type, config, form, user, canChangeStatus, equipment }) {
  ...
  if (type === 'checklists') {
    ...
    const hasDeviation = Object.values(answers).some(answer => ['Condicional', 'Malo'].includes(answer?.value));
...
export default function EntityPage({ type }) {
  ...
  const canReview = canAny(config.reviewPermissions || []);
  const canSendToMaintenance = canAny(config.sendToMaintenancePermissions || []);
  ...
  const completeReview = async action => {
    if (!reviewing) return;
    try {
      await reviewRequest(companyId, reviewing, action, user, reviewComment);
      setToast({ type: 'success', message: 'Solicitud actualizada.' });
