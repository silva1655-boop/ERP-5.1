...
const emptyWorkOrderFilters = { text: '', status: '', priority: '', dateFrom: '', dateTo: '' };
const workOrderPriorityOptions = ['baja', 'media', 'alta', 'critica'];

function workOrderDateValue(workOrder) {
  const value = workOrder.dueDate || workOrder.scheduledAt || workOrder.updatedAt || workOrder.createdAt;
...
function matchesWorkOrderFilters(workOrder, filters) {
  ...
  const date = workOrderDateValue(workOrder);
...
function MechanicWorkOrdersView({ navigationKey }) {
  ...
  const rows = useMemo(() => workOrders.filter(item => isWorkOrderAssignedToUser(item, user) && view.statuses.includes(item.status) && matchesWorkOrderFilters(item, filters)), [filters, workOrders, user, view.statuses]);
  ...
  return <section className="space-y-4">
    ...
    <FilterBar filters={filters} onChange={setFilters} onClear={() => setFilters(emptyWorkOrderFilters)} placeholder="Buscar por equipo, folio, prioridad o texto" statusOptions={view.statuses} priorityOptions={workOrderPriorityOptions}/>
...
function SupervisorFinishedWorkReview() {
  ...
  const [saving, setSaving] = useState(false);
  const [filters, setFilters] = useState(emptyWorkOrderFilters);
  const finished = workOrders.filter(item => item.status === 'finalizada_mecanico' && matchesWorkOrderFilters(item, filters));
  ...
  return <section className="mb-5 space-y-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
    ...
    <FilterBar filters={filters} onChange={setFilters} onClear={() => setFilters(emptyWorkOrderFilters)} placeholder="Buscar por equipo, folio, técnico o texto" statusOptions={['finalizada_mecanico']} priorityOptions={workOrderPriorityOptions}/>
