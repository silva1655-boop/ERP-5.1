...
import { createRequestConvertedToWorkOrderNotification } from './notificationService';
...
export async function createWorkOrderFromMaintenanceRequest(companyId, request, user) {
  ...
  await createRequestConvertedToWorkOrderNotification(companyId, request, { id: ref.id, ...payload }, user);
  return { id: ref.id, ...payload };
}
