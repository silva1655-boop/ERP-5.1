...
export function notificationRecipients(notification = {}) {
  const explicitRoles = notification.recipientRoles || notification.recipientsRoles || notification.targetRoles;
  if (explicitRoles?.length) return explicitRoles;
  if (notification.targetUserId || notification.recipientUserIds?.length) return [];
  if (['work_order_finished', 'work_order_closed'].includes(notification.type)) return ['operaciones', 'supervisor', 'admin_empresa', 'superadmin'];
  if (['work_order_assignment', 'work_order_assigned'].includes(notification.type)) return ['mecanico'];
  if (['finding_created', 'checklist_finding'].includes(notification.type)) return ['operaciones', 'supervisor', 'admin_empresa', 'superadmin'];
  if (notification.type === 'finding_derived_to_maintenance') return ['supervisor', 'admin_empresa', 'superadmin'];
  if (notification.type === 'request_converted_to_work_order') return ['supervisor', 'operaciones', 'admin_empresa', 'superadmin'];
  return [];
}
...
export function notificationTargets(notification = {}, user = null) {
  const explicitTargets = [
    notification.targetNavigationKey,
    ...(notification.targetNavigationKeys || []),
    ...(notification.targetPages || []),
    notification.targetPage,
  ].filter(Boolean);
  if (explicitTargets.length) return [...new Set(explicitTargets)];
  if (['work_order_assignment', 'work_order_assigned'].includes(notification.type)) return ['myWorkOrders'];
  if (notification.type === 'work_order_closed') return user?.role === 'mecanico' ? ['workHistory'] : ['requests'];
  if (notification.type === 'work_order_finished') return user?.role === 'supervisor' ? ['workOrders'] : ['requests'];
  if (['finding_created', 'checklist_finding', 'finding_derived_to_maintenance'].includes(notification.type)) return ['requests'];
  if (notification.type === 'request_converted_to_work_order') return ['workOrders'];
  return ['dashboard'];
}

export function isNotificationUnreadForUser(notification, user) {
  if (!notification || !user || ['archived', 'read'].includes(notification.status)) return false;
...
export async function getUnreadCountByTargetPage(companyId, user) {
  ...
  return unread.reduce((acc, item) => {
    notificationTargets(item, user).forEach(page => { acc[page] = (acc[page] || 0) + 1; });
    return acc;
  }, {});
}
...
export async function markNotificationAsRead(companyId, notificationId, user) {
  ...
  if (!companyId || !notificationId || !userId) return;
  const ref = companyDoc(companyId, 'notifications', notificationId);
  const snapshot = await getDoc(ref);
  const current = snapshot.exists() ? snapshot.data() : {};
  if (notificationReadByIds(current).includes(userId)) return;
  const readEntry = { userId, userName: displayName(user), role: roleOf(user), readAt: new Date().toISOString() };
  await updateDoc(ref, {
    readBy: [...(current.readBy || []), readEntry],
    reviewedBy: userId,
    reviewedByName: displayName(user),
    reviewedByRole: roleOf(user),
