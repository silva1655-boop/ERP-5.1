...
export async function savePreoperationalChecklist({ companyId, template, form, answers, equipment, user }) {
  ...
  await updateEquipmentAfterChecklist(companyId, equipment.id, { hourmeter: Number(form.hourmeter), status: nextEquipmentStatus, checklistId: ref.id, checklistFolio: folio }, user);
